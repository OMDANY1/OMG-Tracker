...
INSERT INTO public.review_routing_rules (
    ...
    'a180f3ab-33bb-4431-88a4-592add7c773e',
    '00a38eae-a90e-4796-89e4-56394e987666', -- Sara
    NULL,                                     -- General / non-Hard clients (Easy, Medium, etc.)
    '8ade760c-c482-4cfb-91e6-dbd8da040c4c', -- Nada (Senior Reviewer)
    'bcfa3baa-7045-4262-abd6-bdb0be8210fd', -- Emad (Owner Fallback)
    15,
    FALSE
)
...
BEGIN
    ...
    LOOP
        ...
        IF v_target_assignee_id IS NULL THEN
            RAISE EXCEPTION 'يجب اختيار مصمم صالح للبوست رقم % قبل إنشاء التاسك.', COALESCE(v_post_number, 'غير محدد');
        END IF;
        ...
        IF NOT EXISTS (
            ...
            RAISE EXCEPTION 'المصمم المختار للبوست رقم % غير موجود أو غير نشط في مساحة العمل.', COALESCE(v_post_number, 'غير محدد');
        END IF;

        -- Determine Reviewer
        -- 1. Check explicit reviewer from payload
        v_explicit_reviewer_id := public.safe_cast_uuid(v_item->>'reviewer_id');
        IF v_explicit_reviewer_id IS NOT NULL AND v_explicit_reviewer_id <> v_target_assignee_id THEN
            IF EXISTS (
                SELECT 1 FROM public.roster_people
                WHERE workspace_id = p_workspace_id AND id = v_explicit_reviewer_id AND is_active = TRUE
            ) THEN
                v_reviewer_id := v_explicit_reviewer_id;
            ELSE
                v_reviewer_id := NULL;
            END IF;
        ELSE
        ...
        -- 2. If not explicitly set, determine Reviewer according to routing rules:
...
');
        ELSE
            v_full_description := COALESCE(v_brief, v_title);
        END IF;
