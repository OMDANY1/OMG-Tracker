-- Migration 23: Enhance get_worker_cron_status with pg_net HTTP responses
-- File: supabase/migrations/20260910000023_enhance_cron_monitoring_with_net_status.sql
...
BEGIN
    ...
    -- 3. Check recent pg_net HTTP responses
    BEGIN
        SELECT jsonb_agg(
            jsonb_build_object(
                'id', id,
    ...
    -- 4. Check if vault secret is configured (without revealing the secret itself!)
    SELECT EXISTS (
    ...
    RETURN jsonb_build_object(
        ...
        'recent_http_responses', COALESCE(v_recent_responses, '[]'::jsonb)
    );
END;
$$;
